...
public class OrderBaseInfoBO extends BaseBO {
	...
	/**
	 * 合买状态;1：招募中；2：已满员；3：合买流产 4：合买撤单,
	 * 前端取值时，已满员和之前逻辑一样，走出票后面的状态，非已满员，根据此字段值显示
	 */
	 ...
	 * 合买信息
	 */
	private  OrderDetailGroupInfoBO orderDetailGroupInfoBO;
	...
	public OrderDetailGroupInfoBO getOrderDetailGroupInfoBO() {
		return orderDetailGroupInfoBO;
	}

	public void setOrderDetailGroupInfoBO(OrderDetailGroupInfoBO orderDetailGroupInfoBO) {
		this.orderDetailGroupInfoBO = orderDetailGroupInfoBO;
	}
	...
	public Double getProfitAmount() {
		if(getWinningStatus().intValue() == 3 || getWinningStatus().intValue()==4){
			if(!ObjectUtil.isBlank(getPreBonus())){
				return NumberUtil.sub(getPreBonus(),getOrderAmount());
			}
		}
		return 0d;
	}
