...
/**
 * @desc 福彩3D近期走势(包括和值走势、号码基本走势、号码不分区走势、大小比走势、奇偶比走势、类型走势等),输出封装
 * @author huangb
 * @date 2017年6月27日
 * @company 益彩网络
 * @version v1.0
 */
public class F3dRecentTrendOutBO extends TrendBaseBO {

	private static final long serialVersionUID = 2375933270718049490L;

	public F3dRecentTrendOutBO() {
	}
	 ...
	 *            源数据
	 */
	public F3dRecentTrendOutBO(F3dRecentTrendBO src) {
		super.setIssue(src.getIssue());
		super.setDrawCode(src.getDrawCode());
		this.drawCodeTest = src.getDrawCodeTest();
		this.sum = src.getSum();
	 ...
	 * @return 封装和值走势的数字集合
	 */
	public List<Integer> genSumDigits(F3dSumTrendBO sumTrend) {
		if (null == sumTrend) {
	 ...
	 * @return 封装大小比走势的数字集合
	 */
	public List<Integer> genBsDigits(F3dBigSmallTrendBO bsTrend) {
		if (null == bsTrend) {
			return new ArrayList<>();
		}
	 ...
	 * @return 封装奇偶比走势的数字集合
	 */
	public List<Integer> genOeDigits(F3dOddEvenTrendBO oeTrend) {
		if (null == oeTrend) {
			return new ArrayList<>();
		}
	 ...
	 * @return 封装类型走势的数字集合
	 */
	public List<Integer> genTypeDigits(F3dTypeTrendBO typeTrend) {
		if (null == typeTrend) {
			return new ArrayList<>();
		}
