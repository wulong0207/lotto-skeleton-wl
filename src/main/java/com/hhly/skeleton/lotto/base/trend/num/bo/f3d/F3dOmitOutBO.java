...
/**
 * @desc 福彩3d 遗漏、冷热、概率，输出封装
 * @author huangb
 * @date 2017年6月29日
 * @company 益彩网络
 * @version v1.0
 */
public class F3dOmitOutBO extends OmitOutBaseBO {

	private static final long serialVersionUID = -2606293984941979468L;
	 ...
	 *            源数据
	 */
	public F3dOmitOutBO(Integer flag, F3dOmitType omitType, TrendBaseBO trend) {
		super.setFlag(flag);
	 ...
	 */
	public List<Integer> genOmitsDigits(F3dOmitType omitType, TrendBaseBO trend) {
		...
		case D_PT:
			...
			if (trend instanceof F3dCode3TrendBO) {
				target = genCode3Digits((F3dCode3TrendBO) trend);
			}
			break;
		case G3_BH_G6_PT_DT:
			// 不分区遗漏;结果集大小：10
			if (trend instanceof F3dTrendBO) {
				target = genBaseDigits((F3dTrendBO) trend);
			}
			break;
		case D_SUM:
			// 和值遗漏;结果集大小：28(0-27)
			if (trend instanceof F3dSumTrendBO) {
				target = genSumDigits((F3dSumTrendBO) trend, Constants.NUM_0, Constants.NUM_27);
			}
			break;
		case G3_SUM:
			// 和值遗漏;结果集大小：26(1-26)
			if (trend instanceof F3dSumTrendBO) {
				target = genSumDigits((F3dSumTrendBO) trend, Constants.NUM_1, Constants.NUM_26);
			}
			break;
		case G6_SUM:
			// 和值遗漏;结果集大小：22(3-24)
			if (trend instanceof F3dSumTrendBO) {
				target = genSumDigits((F3dSumTrendBO) trend, Constants.NUM_3, Constants.NUM_24);
			}
			break;
		default:
			break;
	 ...
	 */
	public List<Integer> genCode3Digits(F3dCode3TrendBO code3Trend) {
		if (null == code3Trend) {
			return new ArrayList<>();
		}
	 ...
	 * @return 封装和值走势的数字集合
	 */
	public List<Integer> genSumDigits(F3dSumTrendBO sumTrend, int startIdx, int endIdx) {
		if (null == sumTrend) {
			return new ArrayList<>();
		}
	 ...
	 */
	public List<Integer> genBaseDigits(F3dTrendBO baseTrend) {
		if (null == baseTrend) {
			return new ArrayList<>();
		}
