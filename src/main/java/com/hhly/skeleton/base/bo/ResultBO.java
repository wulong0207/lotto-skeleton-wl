...
public class ResultBO<T> extends BaseBO {
	...
	/**
	 * @param isApi 是否读取msg-api文件的错误信息
	 * @param success
	 * @param errorCode
	 * @param data
	 * @param arguments
	 *            多个参数，配置文件中的占位符
	 */
	public ResultBO(boolean isApi, int success, String errorCode, T data, Object... arguments) {
		this.success = success;
		this.errorCode = errorCode;
		this.data = data;
		this.message = PropertyUtil.getApiConfigValue(errorCode, arguments);
	}
	 ...
	 * @return
	 */
	public static <T> ResultBO<T> err(T data) {
		return new ResultBO<T>(ERR, FAIL_CODE, data);
	}
	...
	/**
	 * 构建一个错误结果对象，读取msg-api文件的错误信息
	 * 
	 * @return
	 */
	public static <T> ResultBO<T> errApi(T data) {
		return new ResultBO<T>(true, ERR, FAIL_CODE, data);
	}
	 ...
	 * @return
	 */
	public static ResultBO<?> errApi(String errorCode) {
		return new ResultBO<>(true, ERR, errorCode,null);
	}
	/**
	 * 构建一个错误结果对象，读取msg-api文件的错误信息
	 * 
	 * @return
	 */
	public static ResultBO<?> errApi(String errorCode, Object... arguments) {
		return new ResultBO<>(true, ERR, errorCode, null, arguments);
	}
