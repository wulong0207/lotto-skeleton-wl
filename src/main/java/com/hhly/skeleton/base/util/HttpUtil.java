...
public class HttpUtil {
	 ...
	 * @Desc 代理IP
	 */
	public static final String HTTP_PROXYHOST = PropertyUtil.getPropertyValue(PropertyUtil.getEnvPropertiesName(), "http_proxyHost");

	/**
	 * 代理端口
	 */
	public static final String HTTP_PROXYPORT = PropertyUtil.getPropertyValue(PropertyUtil.getEnvPropertiesName(), "http_proxyPort");
	/**
	 * 不使用代理的白名单
	 */
    public static final String HTTP_NONPROXYHOSTS = PropertyUtil.getPropertyValue(PropertyUtil.getEnvPropertiesName(), "http_nonProxyHosts");
    /**
	 * 是否启用代理
	 */
    public static final String HTTP_PROXY_ENABLE = PropertyUtil.getPropertyValue(PropertyUtil.getEnvPropertiesName(), "http_proxy_enable");
	...
	* @param url
	* @param json
	* @param useProxy 是否走代理，true 走代理  ; false不走代理
	* @time: 2018年5月5日 上午11:52:16
	* @return: String 
	*/
	public static String doPostJson(String url, String json,boolean useProxy) {
		CloseableHttpResponse response = null;
		CloseableHttpClient httpClient = null;
		String result = null;
		try {
			...
			StringEntity entityParams = new StringEntity(json);
			entityParams.setContentEncoding("UTF-8");
			entityParams.setContentType("text/json");// 发送json数据需要设置contentType
			httpPost.setEntity(entityParams);

			if (isProxy(url) && useProxy) {
				httpClient = createProxyHttpClient();
			} else {
				httpClient = createHttpClient();
			}
