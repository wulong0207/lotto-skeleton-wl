...
import com.hhly.skeleton.base.common.OperateEnum.defaultAd;
...
public class ArrayUtil extends ArrayUtils {
	...
	public static String[] intersect(String[]... arr) {
		...
		if(statis.size() == 0) return new String[]{};
	...
	public static <T> Map<T, Integer> group(T[] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			...
			if (result.containsKey(arr[i])) {
				result.put(arr[i], result.get(arr[i]) + Constants.NUM_1);
			} else {
				result.put(arr[i], Constants.NUM_1);
			}
		}
		return result;
	}
