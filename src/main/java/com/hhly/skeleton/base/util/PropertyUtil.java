...
public class PropertyUtil {
	...
	private static Properties propApi = null;
	
	public static final Logger LOGGER = LoggerFactory.getLogger(PropertyUtil.class);
	
	private static final String SYS_PROPERTIES_PATH = "msg.properties";
	
	private static final String SYS_API_PROPERTIES_PATH = "msg-api.properties";
	
	public static final String APILICATION_PROPERTIES_PATH = "application.properties";	
	...
	public static String getApiConfigValue(String key,Object ... arguments) {
		propApi = getApiPropertiesInstance();
		try {
			if (propApi != null) {
				String msg = MessageFormat.format(new String(propApi.getProperty(key)),arguments);
				return msg;
			} else {
				return "";
			}
...
//			prop.load(PropertyUtil.class.getClassLoader().getResourceAsStream(SYS_PROPERTIES_PATH));
	...
	private static Properties getApiPropertiesInstance() {
		if (propApi == null) {
		...
		try {
//			prop.load(PropertyUtil.class.getClassLoader().getResourceAsStream(SYS_PROPERTIES_PATH));
			propApi.load(new InputStreamReader(PropertyUtil.class.getClassLoader().getResourceAsStream(SYS_API_PROPERTIES_PATH), "UTF-8"));
		} catch (IOException ex) {
			LOGGER.error(ex.getMessage());
			return null;
		}
	...
	public static String getEnvPropertiesName(){
		if(ObjectUtil.isBlank(env))
			env = getPropertyValue("application.properties", "spring.profiles.active");
