...
public class CdnManagerProxy {
	...
	* @param auth - Auth 对象
	* */
	public CdnManagerProxy(Auth auth) {
		this(auth, "http://fusion.qiniuapi.com");
	}

	private CdnManagerProxy(Auth auth, String server) {
		this.auth = auth;
		this.server = server;
		this.client = new Client();
	}

	public CdnManagerProxy(Auth auth, Client client) {
		this.auth = auth;
	 ...
	 */
	public static String createTimestampAntiLeechUrl(String host, String fileName, final StringMap queryStringMap, String encryptKey, long deadline) throws QiniuException {
		URL urlObj = null;
		try {
